...
export class GameStateManager {
	...
	async getState(): Promise<MultiplayerGameState | null> {
		...
		const stored = await this.room.storage.get<MultiplayerGameState>(GAME_STATE_KEY);
		if (stored) {
			this.state = stored;
		}
		return this.state;
	...
	async saveState(): Promise<void> {
		if (this.state) {
			await this.room.storage.put(GAME_STATE_KEY, this.state);
		}
	}
	...
	async scheduleAfkWarning(playerId: string): Promise<void> {
		...
		await this.room.storage.put('alarm_data', this.alarmData);
		await this.room.storage.setAlarm(warningTime);
	}
	...
	async scheduleAfkTimeout(playerId: string): Promise<void> {
		...
		await this.room.storage.put('alarm_data', this.alarmData);
		await this.room.storage.setAlarm(timeoutTime);
	}
	...
	async scheduleRoomCleanup(): Promise<void> {
		...
		await this.room.storage.put('alarm_data', this.alarmData);
		await this.room.storage.setAlarm(cleanupTime);
	}
	...
	async scheduleGameStart(): Promise<void> {
		...
		await this.room.storage.put('alarm_data', this.alarmData);
		await this.room.storage.setAlarm(startTime);
	}
	...
	async cancelAlarm(): Promise<void> {
		this.alarmData = null;
		await this.room.storage.delete('alarm_data');
	...
	async getAlarmData(): Promise<AlarmData | null> {
		...
		const stored = await this.room.storage.get<AlarmData>('alarm_data');
		if (stored) {
