 ...
 * @param {Uint8Array} dice
 * @param {number} rolls_remaining
 * @param {number} available_categories
 * @returns {any}
 */
export function analyze_turn(dice, rolls_remaining, available_categories) {
    const ptr0 = passArray8ToWasm0(dice, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.analyze_turn(ptr0, len0, rolls_remaining, available_categories);
    if (ret[2]) {
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('dicee_engine_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
